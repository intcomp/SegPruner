...
import math
from torch.utils.benchmark.examples.blas_compare import fill_core_pool
...
def parse_args():
    ...
    parser.add_argument("--frames_root", default="/data/openeqa/frames12_square", help="images path")
    parser.add_argument("--json_path", default="/data/openeqa/open-eqa-v0.json", help="jsons path")
...
def list_first_n_frames(scene_dir):
    if scene_dir.split('/')[-1] == "pose":
        all_imgs = sorted(glob.glob(os.path.join(scene_dir, "*")), key=lambda p: int(os.path.basename(p).split('.')[0]))
    else:
        all_imgs = sorted(glob.glob(os.path.join(scene_dir, "*")),key=lambda p: int(os.path.basename(p).split('-')[0]))
    return all_imgs
...
def answer_one_sample(question_text, image_paths):
    ...
    data_name = image_paths.split('/')[-2]

    if data_name.startswith("hm3d"):
        intrinsic = [[192, 0, 191.5, 0], [0, 228.3468146, 107.5, 0], [0, 0, 1, 0], [0, 0, 0, 1]]
    elif data_name.startswith("scannet"):
...
def main():
    ...
    for idx, ann in zip(pbar, annotations):
        ...
        scene_id = ann.get("episode_history")
        question = ann.get("question").strip()
        gt_answers = ann.get("answer")
    ...
    out_path = os.path.join("/data/openeqa", "answers")
    os.makedirs(out_path, exist_ok=True)
    with open(os.path.join(out_path,f"visual_token_num:{visual_token_num}_r:{important_ratio:.1f}_lam:{lam:.1f}__acc:{acc:.4f}.jsonl"), "w", encoding="utf-8") as f:
        for r in results:
